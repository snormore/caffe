...
caffe_root = "./"
deploy_protocol = caffe_root + "models/dermnet/googlenet_train_val.prototxt"
caffe_model = caffe_root + "models/dermnet/googlenet_dermnet.caffemodel"
image_dictionary_file = caffe_root + "data/dermnet/image_dictionary.txt"
test_images_file = caffe_root + "data/dermnet/test.txt"
test_results_file = caffe_root + "data/dermnet/googlenet_outputs.txt"
...
with open(test_results_file, 'wb') as f:
	...
	# Print to standard output
        ...
        print("Predicted class: #{}.".format(out['prob3'][0].argmax()))
	
	# Print to file
	print >>f, "\nImage: %s" % (test_image[0])
	print >>f, "Actual class: %s." % (test_image[1])
	print >>f, "Predicted class: #{}.".format(out['prob3'][0].argmax())
        
	# Output top-5 prediction 
        scores = zip(image_classes, out['prob3'][0].tolist())
        ...
        if int(test_image[1]) == out['prob3'][0].argmax():
	    accuracy_top1 += 1
	    is_top1 = True
